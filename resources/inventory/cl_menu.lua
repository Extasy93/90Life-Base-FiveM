...
local defaultHeader = {"commonmenuu", "interaction_bgd"}
...
local defaultCheckbox = { [0] = {"commonmenuu", "shop_box_blank"}, [1] = {"commonmenuu", "shop_box_tickb"}, [2] = {"commonmenuu", "shop_box_tick"} }
...
function PMenu:ProcessControl()
	...
	if (boolUP or boolDOWN) and currentButtonsCount and self.Pag[3] then
		if boolDOWN and (self.Pag[3] < currentButtonsCount) or boolUP and (self.Pag[3] > 1) then
			...
			self.Pag = { boolUP and currentButtonsCount - 9 or 1, boolUP and currentButtonsCount or 10, boolDOWN and 1 or currentButtonsCount }
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	DrawText2(0, (button.price and "" or "") .. stringPrefix .. (button.name or ""), .260, intX - intW / 2 + .005, intY - intH / 2 + .0025, tableColor)
	...
	if canUse then
		...
		if button.name and self.Menu[string.lower(button.name)] and not currentMenuData.item and not slideExist then
			--drawSprite("commonmenuutu", "arrowright", intX + (intW / 2.2), intY, .009, .018, 0.0, tableColor[1], tableColor[2], tableColor[3], 255)
			add = .0125
		end
			...
			local max = slideExist and slide and (type(slide.slidemax) == "function" and slide.slidemax(button, self) or slide.slidemax)
				if (max and type(max) == "number" and max > 0 or type(max) == "table" and #max > 0) or not slideExist then
					...
					if boolSelected and slideExist then
						drawSprite("commonmenuu", "arrowright", intX + (intW / 2) - .01025, intY + 0.0004, .009, .018, 0.0, tableColor[1], tableColor[2], tableColor[3], 255)
						button.offset = MeasureStringWidth(slideText, 0, .275)
						drawSprite("commonmenuu", "arrowleft", intX + (intW / 2) - button.offset - .016, intY + 0.0004, .009, .018, 0.0, tableColor[1], tableColor[2], tableColor[3], 255)
					end
					...
					intX = boolSelected and intX - .0260 or intX - .0125
				end
			end
		...
		if button.parentSlider ~= nil then
			...
			drawSprite("mpleaderboard", "leaderboard_female_icon", intX + (intW / 2) - .01025, intY + 0.0004, .0156, .0260, 0.0, tableColor[1], tableColor[2], tableColor[3], 255)
			drawSprite("mpleaderboard", "leaderboard_male_icon", intX - .015, intY + 0.0004, .0156, .0260, 0.0, tableColor[1], tableColor[2], tableColor[3], 255)
		...
		if textBonus and string.len(textBonus) > 0 then
			DrawText2(0, textBonus, .260, intX + (intW / 2) - .005 - add,  intY - intH / 2 + .00375, tableColor, true, 2)
		end
		...
		if Ammo and string.len(Ammo) > 0 then
			drawSprite("commonmenuu", "shop_ammo_icon_b", intX + (intW / 2.15), intY, .02, .034, 0.0, 255, 255, 255, 255)
		end

		local result = (self.Events["image"] and self.Events["image"](button, self.Data.currentMenu, self)) or (button.image and button.image) 
		if result then
			if type(result) == "table" then 
				drawSprite("commonmenuu", result[1], intX + (intW / 2.15), intY, .02, .034, 0.0, result[2], result[3], result[4], 255)
			else
				drawSprite("commonmenuu", result, intX + (intW / 2.15), intY, .02, .034, 0.0, 255, 255, 255, 255)
			end	
		end 

	else
		drawSprite("commonmenuu", "shop_lock", intX + (intW / 2.15), intY, .02, .034, 0.0, tableColor[1], tableColor[2], tableColor[3], 255)
	end
end
...
function PMenu:DrawHeader(intCount)
	...
	if boolHeader then
		...
		drawSprite(parentHeader, childHeader, self.Width - spriteW / 2, self.Height - intH / 2, spriteW, intH, .0, self.Base.HeaderColor[1], self.Base.HeaderColor[2], self.Base.HeaderColor[3], self.Base.HeaderColor[4])
		self.Height = self.Height - 0.03
		if not self.Base.CustomHeader then
			DrawText2(4, self.Base.Title, .6, self.Width  - spriteW / 2, self.Height - intH / 2 + .0125, color_white, false, 0)
		end
	end
	...
	DrawText2(0, firstToUpper(self.Data.currentMenu), .260, self.Width - rectW + .005, self.Height - rectH - 0.0015, color_white, true)
	self.Height = self.Height + 0.005
	DrawText2(0, stringCounter, .260, self.Width - rectW / 2 + .11, self.Height - _intH, color_white, true, 2)
...
function PMenu:DrawHelpers(tableButtons)
	...
	if hasHelp then
		...
		DrawSprite("commonmenuu", "gradient_bgd", self.Width - _intW / 2, self.Height + nwintH / 2 - 0.015, _intW, nwintH, .0, 255, 255, 255, 255)
...
function PMenu:DrawExtra(tableButtons)
	...
	if button and button.opacity ~= nil then
		...
		self.Height =  self.Height - 0.01
		drawSprite("commonmenuu", "gradient_bgd", self.Width - proW / 2, self.Height + proH / 2, proW, proH, 0.0, 255, 255, 255, 255)
		self.Height = self.Height + 0.005
		DrawText2(0, "0%", 0.260, self.Width - _intW + .005, self.Height, color_white, false, 1)
		DrawText2(0, "Opacité", 0.260, self.Width - _intW / 2, self.Height, color_white, false, 0)
		DrawText2(0, "100%", 0.260, self.Width - 0.005, self.Height, color_white, false, 2)
	...
	if button and button.advSlider ~= nil then
		...
		DrawText2(0, tostring(button.advSlider[1]), 0.260, self.Width - _intW + .005, self.Height, color_white, false, 1)
		DrawText2(0, "Variations disponibles", 0.260, self.Width - _intW / 2, self.Height, color_white, false, 0)
		DrawText2(0, tostring(button.advSlider[2]), 0.260, self.Width - 0.005, self.Height, color_white, false, 2)

		self.Height = self.Height + .03
		drawSprite("commonmenuu", "arrowright", self.Width - 0.01, self.Height, .015, .03, 0.0, 255, 255, 255, 255)
		drawSprite("commonmenuu", "arrowleft", self.Width - proW + 0.01, self.Height, .015, .03, 0.0, 255, 255, 255, 255)
...
function PMenu:Draw()
	...
	if tableButtons and intCount and not self.Invisible then
		...
		if currentMenu and (currentMenu.extra or currentButton and currentButton.advSlider) then
