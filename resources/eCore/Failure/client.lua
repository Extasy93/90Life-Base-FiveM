...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			local entityFromNetID = (netID)
			local plate = GetVehicleNumberPlateText(vehicle)
			
			if entityFromNetID ~= lastidse then
				TriggerServerEvent('Extasy:duplicarcheck', token, plate, entityFromNetID)
				lastidse = entityFromNetID
			end
		end
					...
					if mechNumb == 1 then
						...
						if GetIsVehicleEngineRunning(vehicle) then
							...
							if cfg.chargeForRepairs then
								TriggerServerEvent('rvFailure:takemoney', token, repairCost)
	...
	Citizen.CreateThread(function()
		while true do
			...
			if pedInSameVehicleLast then
				local factor = 1.0
				if healthEngineNew < cfg.engineSafeGuard + 5 then
...
RegisterNetEvent('Extasy:achatvehcar')
AddEventHandler('Extasy:achatvehcar', function(statut)
  achatveh = statut
end)
...
function CKC()
	Citizen.CreateThread(function()
		...
		while isPedDrivingAVehicle() and ckc do
			...
			countglitch = countglitch + 1

			if countglitch > 500 then
				vitesse = GetEntitySpeed(vehicle)
				--print(vitesse)
				if vitesse > 1.0 then
					local ped = GetPlayerPed(-1)
					local co = GetEntityCoords(ped)
					SetEntityCoords(ped, co, true, true, true)
					Citizen.Wait(1)
...
Citizen.CreateThread(function()
	while true do
		...
		local distancea  = GetDistanceBetweenCoords(coords, -783.25280761719, -223.4813079834, 37.32152557373, true)
		local distanceb  = GetDistanceBetweenCoords(coords, -760.975, -228.845, 36.284, true)
		local distancec = GetDistanceBetweenCoords(coords, -438.35430908203, -2180.8576660156, 9.324508666992, true)
		local distanced = GetDistanceBetweenCoords(coords, -791.958, -223.963, 36.08, true)
		local distancee = GetDistanceBetweenCoords(coords, -761.034, -230.709, 35.669, true)

		if distancea > 10 and distanceb > 10 and distancec > 10 and distanced > 10 and distancee > 10 then
				desactmoldu = true
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			-- -783.25280761719, y = -223.4813079834, z = 37.32152557373
			-- x=-760.975, y=-228.845, z=36.284
			local coords    = GetEntityCoords(playerPed)

			--{x=-791.958, y=-223.963, z=36.08} {h=114.904} {Comment: coords},
			--{x=-761.034, y=-230.709, z=35.669} {h=209.275} {Comment: coords1},

			local plate = ESX.Math.Trim(GetVehicleNumberPlateText(vehicle))


			if healthEngineCurrent <= 200 and not ckc then
				--SetEntityMaxSpeed(vehicle, 0)
				ckc = true
				CKC()
				TriggerEvent('Extasy:showNotification', 'Tu as cassé ta voiture ! Appelle un mécanicien...')
			else
				if healthEngineCurrent > 200 then
					ckc = false
				end
			end
