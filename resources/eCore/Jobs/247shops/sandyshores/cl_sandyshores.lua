...
loadSandyShores = function()
    SANDYSHORES = {}

    SANDYSHORES.openMainMenu = function()
        RMenu.Add('hawick_menu', 'main_menu', RageUI.CreateMenu("24/7 Sandy Shores", "Que voulez-vous faire ?", 1, 100))
        RMenu.Add('hawick_menu', 'main_citizen_billing', RageUI.CreateSubMenu(RMenu:Get('hawick_menu', 'main_menu'), "24/7 Sandy Shores", "Que voulez-vous faire ?"))
            ...
            Citizen.CreateThread(function()
                while hawick_inmenu do
                    ...
                    RageUI.IsVisible(RMenu:Get('hawick_menu', 'main_menu'), true, false, true, function()
                        ...
                        RageUI.Button("Annonce 24/7 Sandy Shores", nil, {RightLabel = "→→→"}, true, function(h, a, s) 
                            if s then
                                ...
                                if a ~= nil then
                                    if string.sub(a, 1, string.len("-")) == "-" then
                                        ...
                                        if string.len(a) > 5 then
                                            TriggerServerEvent("jobCounter:sendAllAdvancedMessageWithoutShit", token, '24/7 Sandy Shores', 'Publicité', a, 'CHAR_247SANDYSHORES', 8, "24/7 Sandy Shores")
                                        end
                                    end
                                else
                        ...
                        if IsPatron(playerJob, tonumber(playerJob_grade)) then
                            for k,v in pairs(cfg_sandyshores.blip) do
                                            ...
                                            if i ~= nil then
                                                if i > 0 then
                                                    if string.sub(i, 1, string.len("-")) == "-" then
                                                        ...
                                                        Wait(2500)
                                                        TriggerServerEvent('247shops:sendBilling', token, "Facture 24/7 Sandy Shores", i, "24/7 Sandy Shores", GetPlayerServerId(player), false, nil, i * 2.5)
    ...
    registerSocietyFarmZone({
        pos           = vector3(1967.15, 3739.9, 32.35),
        spawnPoint    = {
            {pos = vector3(1964.77, 3732.9, 32.32), heading = 120.0},
        },
        type          = 5,
        msg           = "~p~Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le garage de l'épicerie",
        garage        = {
    ...
    registerSocietyFarmZone({
        pos           = vector3(1964.77, 3732.9, 32.32),
        type          = 7,
    ...
    AddTextComponentString("Gestion entreprise 24/7 Sandy Shores")
    EndTextCommandSetBlipName(aBlip)

    registerSocietyFarmZone({
        pos      = vector3(1958.88, 3748.29, 32.34),
        type     = 9,
        size     = 1.5,
        marker   = true,
    ...
    SANDYSHORES.openCatalogMenu = function()
        RMenu.Add('hawick_menu_catalog', 'main_menu', RageUI.CreateMenu("24/7 Sandy Shores", "Que voulez-vous faire ?", 1, 100))
        RMenu:Get('hawick_menu_catalog', 'main_menu').Closed = function()
            hawick_inmenu = false
    
            RMenu:Delete('hawick_menu_catalog', 'main_menu')

            for k,v in pairs(cfg_sandyshores.catalog) do
                RMenu:Delete('hawick_menu_catalog', v.value)
            end
        end

        for k,v in pairs(cfg_sandyshores.catalog) do
            RMenu.Add('hawick_menu_catalog', v.value, RageUI.CreateSubMenu(RMenu:Get('hawick_menu_catalog', 'main_menu'), "24/7 Sandy Shores", "Que voulez-vous faire ?"))
        end
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for k,v in pairs(cfg_sandyshores.catalogPos) do
