loadGroveStreet = function()
    GROVESTREET = {}

    GROVESTREET.openMainMenu = function()
        RMenu.Add('grovestreet_menu', 'main_menu', RageUI.CreateMenu("24/7 Grove Street", "Que voulez-vous faire ?", 1, 100))
        RMenu.Add('grovestreet_menu', 'main_citizen_billing', RageUI.CreateSubMenu(RMenu:Get('grovestreet_menu', 'main_menu'), "24/7 Grove Street", "Que voulez-vous faire ?"))
        RMenu.Add('grovestreet_menu', 'main_setup_logs', RageUI.CreateSubMenu(RMenu:Get('grovestreet_menu', 'main_menu'), "24/7 Grove Street", "Que voulez-vous faire ?"))
        RMenu:Get('grovestreet_menu', 'main_menu').Closed = function()
            grovestreet_inmenu = false
    
            RMenu:Delete('grovestreet_menu', 'main_menu')
            RMenu:Delete('grovestreet_menu', 'main_citizen_billing')
        end
        ...
        if not settings.color[1] or not settings.color[2] or not settings.color[3] then
            for name, menu in pairs(RMenu['grovestreet_menu']) do
                RMenu:Get('grovestreet_menu', name):SetRectangleBanner(72, 0, 255, 100)
            end
        else
            for name, menu in pairs(RMenu['grovestreet_menu']) do
                RMenu:Get('grovestreet_menu', name):SetRectangleBanner(settings.color[1], settings.color[2], settings.color[3], 100)
            end
        end
        
        if grovestreet_inmenu then
            grovestreet_inmenu = false
            return
        else
            grovestreet_inmenu = true
            RageUI.Visible(RMenu:Get('grovestreet_menu', 'main_menu'), true)
            Citizen.CreateThread(function()
                while grovestreet_inmenu do
                    Wait(1)
                    RageUI.IsVisible(RMenu:Get('grovestreet_menu', 'main_menu'), true, false, true, function()
                        ...
                        RageUI.Button("Facturation", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) end, RMenu:Get('grovestreet_menu', 'main_citizen_billing'))                           
                        RageUI.Button("Annonce 24/7 GroveStreet", nil, {RightLabel = "→→→"}, true, function(h, a, s) 
                            if s then
                                ...
                                if a ~= nil then
                                    if string.sub(a, 1, string.len("-")) == "-" then
                                        ...
                                        if string.len(a) > 5 then
                                            TriggerServerEvent("jobCounter:sendAllAdvancedMessageWithoutShit", token, '24/7 Grove Street', 'Publicité', a, 'CHAR_247GROVESTREET', 8, "24/7 Grove Street")
                                        end
                                    end
                                else
                        ...
                        if IsPatron(playerJob, tonumber(playerJob_grade)) then
                            for k,v in pairs(cfg_grovestreet.blip) do
                                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), v.pos, false) < 50.0 then
                                    ...
                                    end, RMenu:Get('grovestreet_menu', 'main_setup_logs'))                           
                                else
                    ...
                    RageUI.IsVisible(RMenu:Get('grovestreet_menu', 'main_setup_logs'), true, false, true, function()
                    ...
                    RageUI.IsVisible(RMenu:Get('grovestreet_menu', 'main_citizen_billing'), true, false, true, function()
                                            ...
                                            if i ~= nil then
                                                if i > 0 then
                                                    if string.sub(i, 1, string.len("-")) == "-" then
                                                        ...
                                                        Wait(2500)
                                                        TriggerServerEvent('247shops:sendBilling', token, "Facture 24/7 Grove Street", i, "24/7 Grove Street", GetPlayerServerId(player), false, nil, i * 2.5)
    ...
    registerSocietyFarmZone({
        pos           = vector3(-40.94, -1747.9, 29.33),
        spawnPoint    = {
            {pos = vector3(-46.62, -1738.55, 29.15), heading = 44.99,}
        },
        type          = 5,
    ...
    registerSocietyFarmZone({
        pos           = vector3(-46.62, -1738.55, 29.15),
        type          = 7,
    ...
    AddTextComponentString("Gestion entreprise 24/7 GroveStreet")
    EndTextCommandSetBlipName(aBlip)

    registerSocietyFarmZone({
        pos      = vector3(-43.93, -1750.42, 29.42),
        type     = 9,
        size     = 1.5,
        marker   = true,
    ...
    GROVESTREET.openCatalogMenu = function()
        RMenu.Add('grovestreet_menu_catalog', 'main_menu', RageUI.CreateMenu("24/7 GroveStreet", "Que voulez-vous faire ?", 1, 100))
        RMenu:Get('grovestreet_menu_catalog', 'main_menu').Closed = function()
            grovestreet_inmenu = false
    
            RMenu:Delete('grovestreet_menu_catalog', 'main_menu')

            for k,v in pairs(cfg_grovestreet.catalog) do
                RMenu:Delete('grovestreet_menu_catalog', v.value)
            end
        end

        for k,v in pairs(cfg_grovestreet.catalog) do
            RMenu.Add('grovestreet_menu_catalog', v.value, RageUI.CreateSubMenu(RMenu:Get('grovestreet_menu_catalog', 'main_menu'), "24/7 GroveStreet", "Que voulez-vous faire ?"))
        end
        ...
        if not settings.color[1] or not settings.color[2] or not settings.color[3] then
            for name, menu in pairs(RMenu['grovestreet_menu_catalog']) do
                RMenu:Get('grovestreet_menu_catalog', name):SetRectangleBanner(72, 0, 255, 100)
            end
        else
            for name, menu in pairs(RMenu['grovestreet_menu_catalog']) do
                RMenu:Get('grovestreet_menu_catalog', name):SetRectangleBanner(settings.color[1], settings.color[2], settings.color[3], 100)
            end
        end
        ...
        if grovestreet_inmenu then
            ...
            grovestreet_inmenu = true
            RageUI.Visible(RMenu:Get('grovestreet_menu_catalog', 'main_menu'), true)
            Citizen.CreateThread(function()
                while grovestreet_inmenu do
                    Wait(1)
                    RageUI.IsVisible(RMenu:Get('grovestreet_menu_catalog', 'main_menu'), true, false, true, function()
                        ...
                        for k,v in pairs(cfg_grovestreet.catalog) do
                            RageUI.Button(v.name, nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) end, RMenu:Get('grovestreet_menu_catalog', v.value))                           
                        end
                    ...
                    for k,v in pairs(cfg_grovestreet.catalog) do
                        RageUI.IsVisible(RMenu:Get('grovestreet_menu_catalog', v.value), true, false, true, function()
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for k,v in pairs(cfg_grovestreet.catalogPos) do
