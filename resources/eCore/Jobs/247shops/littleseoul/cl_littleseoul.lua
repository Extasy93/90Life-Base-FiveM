...
loadLittleSeoul = function()
    LITTLESEOUL = {}

    LITTLESEOUL.openMainMenu = function()
        RMenu.Add('littleseoul_menu', 'main_menu', RageUI.CreateMenu("24/7 Little Seoul", "Que voulez-vous faire ?", 1, 100))
        RMenu.Add('littleseoul_menu', 'main_citizen_billing', RageUI.CreateSubMenu(RMenu:Get('littleseoul_menu', 'main_menu'), "24/7 Little Seoul", "Que voulez-vous faire ?"))
        RMenu.Add('littleseoul_menu', 'main_setup_logs', RageUI.CreateSubMenu(RMenu:Get('littleseoul_menu', 'main_menu'), "24/7 Little Seoul", "Que voulez-vous faire ?"))
        RMenu:Get('littleseoul_menu', 'main_menu').Closed = function()
            littleseoul_inmenu = false
    
            RMenu:Delete('littleseoul_menu', 'main_menu')
            RMenu:Delete('littleseoul_menu', 'main_citizen_billing')
        end
        ...
        if not settings.color[1] or not settings.color[2] or not settings.color[3] then
            for name, menu in pairs(RMenu['littleseoul_menu']) do
                RMenu:Get('littleseoul_menu', name):SetRectangleBanner(72, 0, 255, 100)
            end
        else
            for name, menu in pairs(RMenu['littleseoul_menu']) do
                RMenu:Get('littleseoul_menu', name):SetRectangleBanner(settings.color[1], settings.color[2], settings.color[3], 100)
            end
        end
        
        if littleseoul_inmenu then
            littleseoul_inmenu = false
            return
        else
            littleseoul_inmenu = true
            RageUI.Visible(RMenu:Get('littleseoul_menu', 'main_menu'), true)
            Citizen.CreateThread(function()
                while littleseoul_inmenu do
                    Wait(1)
                    RageUI.IsVisible(RMenu:Get('littleseoul_menu', 'main_menu'), true, false, true, function()
                        ...
                        RageUI.Button("Facturation", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) end, RMenu:Get('littleseoul_menu', 'main_citizen_billing'))                           
                        RageUI.Button("Annonce 24/7 Little Seoul", nil, {RightLabel = "→→→"}, true, function(h, a, s) 
                            if s then
                                ...
                                if a ~= nil then
                                    if string.sub(a, 1, string.len("-")) == "-" then
                                        ...
                                        if string.len(a) > 5 then
                                            TriggerServerEvent("jobCounter:sendAllAdvancedMessageWithoutShit", token, '24/7 Little Seoul', 'Publicité', a, 'CHAR_247LITTLESEOUL', 8, "24/7 Little Seoul")
                                        end
                                    end
                                else
                        ...
                        if IsPatron(playerJob, tonumber(playerJob_grade)) then
                            for k,v in pairs(cfg_littleseoul.blip) do
                                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), v.pos, false) < 50.0 then
                                    ...
                                    end, RMenu:Get('littleseoul_menu', 'main_setup_logs'))                           
                                else
                    ...
                    RageUI.IsVisible(RMenu:Get('littleseoul_menu', 'main_setup_logs'), true, false, true, function()
                    ...
                    RageUI.IsVisible(RMenu:Get('littleseoul_menu', 'main_citizen_billing'), true, false, true, function()
                                            ...
                                            if i ~= nil then
                                                if i > 0 then
                                                    if string.sub(i, 1, string.len("-")) == "-" then
                                                        ...
                                                        Wait(2500)
                                                        TriggerServerEvent('247shops:sendBilling', token, "Facture 24/7 Little Seoul", i, "24/7 Little Seoul", GetPlayerServerId(player), false, nil, i * 2.5)
    ...
    registerSocietyFarmZone({
        pos           = vector3(-702.85, -916.99, 19.21),
        spawnPoint    = {
            {pos = vector3(-703.62, -921.57, 19.01), heading = 100.0},
        },
        type          = 5,
    ...
    registerSocietyFarmZone({
        pos           = vector3(-703.62, -921.57, 19.01),
        type          = 7,
    ...
    AddTextComponentString("Gestion entreprise 24/7 Little Seoul")
    EndTextCommandSetBlipName(aBlip)

    registerSocietyFarmZone({
        pos      = vector3(-705.22, -914.21, 19.22),
        type     = 9,
        size     = 1.5,
        marker   = true,
    ...
    LITTLESEOUL.openCatalogMenu = function()
        RMenu.Add('littleseoul_menu_catalog', 'main_menu', RageUI.CreateMenu("24/7 LittleSeoul", "Que voulez-vous faire ?", 1, 100))
        RMenu:Get('littleseoul_menu_catalog', 'main_menu').Closed = function()
            littleseoul_inmenu = false
    
            RMenu:Delete('littleseoul_menu_catalog', 'main_menu')

            for k,v in pairs(cfg_littleseoul.catalog) do
                RMenu:Delete('littleseoul_menu_catalog', v.value)
            end
        end

        for k,v in pairs(cfg_littleseoul.catalog) do
            RMenu.Add('littleseoul_menu_catalog', v.value, RageUI.CreateSubMenu(RMenu:Get('littleseoul_menu_catalog', 'main_menu'), "24/7 LittleSeoul", "Que voulez-vous faire ?"))
        end
        ...
        if not settings.color[1] or not settings.color[2] or not settings.color[3] then
            for name, menu in pairs(RMenu['littleseoul_menu_catalog']) do
                RMenu:Get('littleseoul_menu_catalog', name):SetRectangleBanner(72, 0, 255, 100)
            end
        else
            for name, menu in pairs(RMenu['littleseoul_menu_catalog']) do
                RMenu:Get('littleseoul_menu_catalog', name):SetRectangleBanner(settings.color[1], settings.color[2], settings.color[3], 100)
            end
        end
        ...
        if littleseoul_inmenu then
            ...
            littleseoul_inmenu = true
            RageUI.Visible(RMenu:Get('littleseoul_menu_catalog', 'main_menu'), true)
            Citizen.CreateThread(function()
                while littleseoul_inmenu do
                    Wait(1)
                    RageUI.IsVisible(RMenu:Get('littleseoul_menu_catalog', 'main_menu'), true, false, true, function()
                        ...
                        for k,v in pairs(cfg_littleseoul.catalog) do
                            RageUI.Button(v.name, nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) end, RMenu:Get('littleseoul_menu_catalog', v.value))                           
                        end
                    ...
                    for k,v in pairs(cfg_littleseoul.catalog) do
                        RageUI.IsVisible(RMenu:Get('littleseoul_menu_catalog', v.value), true, false, true, function()
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for k,v in pairs(cfg_littleseoul.catalogPos) do
