...
loadStrawberry = function()
    STRAWBERRY = {}

    STRAWBERRY.openMainMenu = function()
        RMenu.Add('strawberry_menu', 'main_menu', RageUI.CreateMenu("24/7 Strawberry", "Que voulez-vous faire ?", 1, 100))
        RMenu.Add('strawberry_menu', 'main_citizen_billing', RageUI.CreateSubMenu(RMenu:Get('strawberry_menu', 'main_menu'), "24/7 Strawberry", "Que voulez-vous faire ?"))
        RMenu.Add('strawberry_menu', 'main_setup_logs', RageUI.CreateSubMenu(RMenu:Get('strawberry_menu', 'main_menu'), "24/7 Strawberry", "Que voulez-vous faire ?"))
        RMenu:Get('strawberry_menu', 'main_menu').Closed = function()
            strawberry_inmenu = false
    
            RMenu:Delete('strawberry_menu', 'main_menu')
            RMenu:Delete('strawberry_menu', 'main_citizen_billing')
        end
        ...
        if not settings.color[1] or not settings.color[2] or not settings.color[3] then
            for name, menu in pairs(RMenu['strawberry_menu']) do
                RMenu:Get('strawberry_menu', name):SetRectangleBanner(72, 0, 255, 100)
            end
        else
            for name, menu in pairs(RMenu['strawberry_menu']) do
                RMenu:Get('strawberry_menu', name):SetRectangleBanner(settings.color[1], settings.color[2], settings.color[3], 100)
            end
        end
        
        if strawberry_inmenu then
            strawberry_inmenu = false
            return
        else
            strawberry_inmenu = true
            RageUI.Visible(RMenu:Get('strawberry_menu', 'main_menu'), true)
            Citizen.CreateThread(function()
                while strawberry_inmenu do
                    Wait(1)
                    RageUI.IsVisible(RMenu:Get('strawberry_menu', 'main_menu'), true, false, true, function()
                        ...
                        RageUI.Button("Facturation", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) end, RMenu:Get('strawberry_menu', 'main_citizen_billing'))                           
                        RageUI.Button("Annonce 24/7 Strawberry", nil, {RightLabel = "→→→"}, true, function(h, a, s) 
                            if s then
                                ...
                                if a ~= nil then
                                    if string.sub(a, 1, string.len("-")) == "-" then
                                        ...
                                        if string.len(a) > 5 then
                                            TriggerServerEvent("jobCounter:sendAllAdvancedMessageWithoutShit", token, '24/7 Strawberry', 'Publicité', a, 'CHAR_247STRAWBERRY', 8, "24/7 Strawberry")
                                        end
                                    end
                                else
                        ...
                        if IsPatron(playerJob, tonumber(playerJob_grade)) then
                            for k,v in pairs(cfg_strawberry.blip) do
                                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), v.pos, false) < 50.0 then
                                    ...
                                    end, RMenu:Get('strawberry_menu', 'main_setup_logs'))                           
                                else
                    ...
                    RageUI.IsVisible(RMenu:Get('strawberry_menu', 'main_setup_logs'), true, false, true, function()
                    ...
                    RageUI.IsVisible(RMenu:Get('strawberry_menu', 'main_citizen_billing'), true, false, true, function()
                                            ...
                                            if i ~= nil then
                                                if i > 0 then
                                                    if string.sub(i, 1, string.len("-")) == "-" then
                                                        ...
                                                        Wait(2500)
                                                        TriggerServerEvent('247shops:sendBilling', token, "Facture 24/7 Strawberry", i, "24/7 Strawberry", GetPlayerServerId(player), false, nil, i * 2.5)
    ...
    registerSocietyFarmZone({
        pos           = vector3(31.59, -1314.7, 29.52),
        spawnPoint    = {
            {pos = vector3(26.59, -1305.42, 29.0), heading = 270.0},
        },
        type          = 5,
    ...
    registerSocietyFarmZone({
        pos           = vector3(26.59, -1305.42, 29.0),
        type          = 7,
    ...
    AddTextComponentString("Gestion entreprise 24/7 Strawberry")
    EndTextCommandSetBlipName(aBlip)

    registerSocietyFarmZone({
        pos      = vector3(24.18, -1347.02, 29.5),
        type     = 9,
        size     = 1.5,
        marker   = true,
    ...
    STRAWBERRY.openCatalogMenu = function()
        RMenu.Add('strawberry_menu_catalog', 'main_menu', RageUI.CreateMenu("24/7 Strawberry", "Que voulez-vous faire ?", 1, 100))
        RMenu:Get('strawberry_menu_catalog', 'main_menu').Closed = function()
            strawberry_inmenu = false
    
            RMenu:Delete('strawberry_menu_catalog', 'main_menu')

            for k,v in pairs(cfg_strawberry.catalog) do
                RMenu:Delete('strawberry_menu_catalog', v.value)
            end
        end

        for k,v in pairs(cfg_strawberry.catalog) do
            RMenu.Add('strawberry_menu_catalog', v.value, RageUI.CreateSubMenu(RMenu:Get('strawberry_menu_catalog', 'main_menu'), "24/7 Strawberry", "Que voulez-vous faire ?"))
        end
        ...
        if not settings.color[1] or not settings.color[2] or not settings.color[3] then
            for name, menu in pairs(RMenu['strawberry_menu_catalog']) do
                RMenu:Get('strawberry_menu_catalog', name):SetRectangleBanner(72, 0, 255, 100)
            end
        else
            for name, menu in pairs(RMenu['strawberry_menu_catalog']) do
                RMenu:Get('strawberry_menu_catalog', name):SetRectangleBanner(settings.color[1], settings.color[2], settings.color[3], 100)
            end
        end
        ...
        if strawberry_inmenu then
            ...
            strawberry_inmenu = true
            RageUI.Visible(RMenu:Get('strawberry_menu_catalog', 'main_menu'), true)
            Citizen.CreateThread(function()
                while strawberry_inmenu do
                    Wait(1)
                    RageUI.IsVisible(RMenu:Get('strawberry_menu_catalog', 'main_menu'), true, false, true, function()
                        ...
                        for k,v in pairs(cfg_strawberry.catalog) do
                            RageUI.Button(v.name, nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected) end, RMenu:Get('strawberry_menu_catalog', v.value))                           
                        end
                    ...
                    for k,v in pairs(cfg_strawberry.catalog) do
                        RageUI.IsVisible(RMenu:Get('strawberry_menu_catalog', v.value), true, false, true, function()
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for k,v in pairs(cfg_strawberry.catalogPos) do
