...
function LeaveInstance()
	if instance.host then
		if #instance.players > 1 then
			ESX.ShowNotification('Quit l\'instance')
		end
...
AddEventHandler('instance:onPlayerEntered', function(_instance, player)
	...
	ESX.ShowNotification('la personne :'..playerName..' a renterer la maison')
end)
...
AddEventHandler('instance:onPlayerLeft', function(_instance, player)
	...
	ESX.ShowNotification('la personne :'..playerName..' a quiter la maison')
end)
...
AddEventHandler('instance:onInvite', function(_instance, type, data)
	...
	Citizen.CreateThread(function()
		...
		if instanceInvite then
			ESX.ShowNotification('invitation expirer')
			instanceInvite = nil
		end
	end)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if instanceInvite then
			ESX.ShowHelpNotification('E pour enter')

			if IsControlJustReleased(0, 38) then
				EnterInstance(instanceInvite)
				ESX.ShowNotification('tu est entrer')
				instanceInvite = nil
			end
		else
			Citizen.Wait(500)
