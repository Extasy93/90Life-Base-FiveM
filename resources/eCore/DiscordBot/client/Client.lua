...
AddEventHandler('playerSpawned', function()
    if firstSpawn then
        for _, v in pairs(DiscordLOG.Buttons) do
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'pistoled' or DeathReason == 'Invalid' or DeathReason == 'murdered' or DeathReason == 'mowed over' or DeathReason == 'knifed' or DeathReason == 'flattened' then
				TriggerServerEvent('DiscordBot:playerDied', token, '[ID:'..GetPlayerServerId(Killer)..'] '.. GetPlayerName(Killer) .. ' ' .. DeathReason .. ' ' .. '[ID:' .. GetPlayerServerId(PlayerId()) .. '] '.. GetPlayerName(PlayerId()) .. '.', Weapon)
			elseif DeathReason == 'IsRifle' then
				TriggerServerEvent('DiscordBot:playerDied', token, '[ID:'..GetPlayerServerId(Killer)..'] '.. GetPlayerName(Killer) .. ' ' .. DeathReason .. ' ' .. '[ID:' .. GetPlayerServerId(PlayerId()) .. '] '.. GetPlayerName(PlayerId()) .. '.', Weapon)
				Citizen.Wait(2000)
				Extasy.ShowAdvancedNotification("SPECTATEUR", token, "~y~Vape Anti-Cheat", "Un report a été envoyer au Administrateur en ligne pour évalué la raison de votre mort !", "CHAR_LESTER_DEATHWISH", 3)
				ExecuteCommand('report Je suis mort de facon bizzard ta capter ? Vas voir si je me suis pas fait niké pas un moddeur...')
			else
				TriggerServerEvent('DiscordBot:playerDied', token, '[ID:'..GetPlayerServerId(Killer)..'] '.. GetPlayerName(Killer) .. ' ' .. DeathReason .. ' ' .. '[ID:' .. GetPlayerServerId(PlayerId()) .. '] '.. GetPlayerName(PlayerId()) .. '.', Weapon)
			end
