...
AddEventHandler("entityCreating", function(token, entity)
	if not CheckToken(token, source, "entityCreating") then return end
...
local weaponsListXD = {
	...
	[GetHashKey("weapon_carbinerifle_mk2")] = {name = "weapon_carbinerifle_mk2"},
...
AddEventHandler("weaponDamageEvent", function(token, source, value)
	if not CheckToken(token, source, "weaponDamageEvent") then return end
...
RegisterNetEvent("eCore:CheckIfJobIsValid")
AddEventHandler("eCore:CheckIfJobIsValid", function(token, job)
	if CheckToken(token, source, "eCore:CheckIfJobIsValid") then
