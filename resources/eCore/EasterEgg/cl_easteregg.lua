      ...
      for k,v in pairs(pickupsData) do
        ...
        if not playerData.pickups[k] and entities[k] then
          if HasPickupBeenCollected(entities[k]) then
            ...
            TriggerServerEvent('Extasy:takePuckup', token, k)
...
RegisterNetEvent('Extasy:receiveData')
AddEventHandler('Extasy:receiveData', function(receivePlayerData, receivePickupsData)
    playerData = receivePlayerData
    pickupsData = receivePickupsData
end)

RegisterNetEvent('EG:Notification')
AddEventHandler('EG:Notification', function(text)
    Extasy.ShowNotification(text)
end)

TriggerServerEvent('Extasy:requestData', token)