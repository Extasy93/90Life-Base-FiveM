...
AddEventHandler('eDatastore:getDataStore', function(token, name, owner, cb)
	if not CheckToken(token, source, "eDatastore:getDataStore") then return end
	cb(GetDataStore(name, owner))
end)

AddEventHandler('eDatastore:getDataStoreOwners', function(token, name, cb)
	if not CheckToken(token, source, "eDatastore:getDataStoreOwners") then return end
	cb(GetDataStoreOwners(name))
end)

AddEventHandler('eDatastore:getSharedDataStore', function(token, name, cb)
	if not CheckToken(token, source, "eDatastore:getSharedDataStore") then return end
