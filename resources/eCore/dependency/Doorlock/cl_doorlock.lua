...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_doorlock:getDoorInfo', function(doorInfo)
		for doorID,state in pairs(doorInfo) do
			cfg_doorlock.DoorList[doorID].locked = state
		end
	end)
end)
...
Citizen.CreateThread(function()
	while true do
		for _,doorID in ipairs(cfg_doorlock.DoorList) do
...
Citizen.CreateThread(function()
	while true do
		...
		for k,doorID in ipairs(cfg_doorlock.DoorList) do
			local distance
			...
			local maxDistance, size, displayText = 1.25, 1, '~g~Ouvert~s~'
			...
			if distance < maxDistance then
				...
				if IsControlJustReleased(0, 38) then
					if isAuthorized then
						...
						TriggerServerEvent('esx_doorlock:updateState', token, k, doorID.locked) -- Broadcast new state of the door to everyone
					end
				end
			end
		end
