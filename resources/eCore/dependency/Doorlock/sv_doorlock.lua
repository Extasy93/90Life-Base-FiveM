...
AddEventHandler('esx_doorlock:updateState', function(token, doorID, state)
	if not CheckToken(token, source, "esx_doorlock:updateState") then return end
	...
	if not cfg_doorlock.DoorList[doorID] then
		print(('esx_doorlock: %s attempted to update invalid door!'):format(xPlayer.identifier))
		return
	end

	if not IsAuthorized(xPlayer.job.name, cfg_doorlock.DoorList[doorID]) then
