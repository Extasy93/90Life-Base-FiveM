...
function takeHostage()
	...
	if not canTakeHostage then
		drawNativeNotification("Vous n'avez pas d'arme ou pas de munitions.")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			holdingHostage = true
			TriggerServerEvent('cmg3_animations:syncExtasyyy', token, closestPlayer, lib,lib2, anim1, anim2, distans, distans2, height,target,length,spin,controlFlagMe,controlFlagTarget,animFlagTarget,attachFlag)
		else
			drawNativeNotification("Aucun joueur a proximité")
		end
	end
	...
	while true do
		...
		if holdingHostage then
			...
			if IsEntityDead(GetPlayerPed(-1)) then
				...
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
				TriggerServerEvent("cmg3_animations:stop", token, target)
				Wait(100)
				releaseHostage()
			end
			...
			DrawText3Dotage(playerCoords.x,playerCoords.y,playerCoords.z,"[E] pour relâcher\n[C] pour tuer")
			if IsDisabledControlJustPressed(0,38) then --release
				...
				Wait(100)
				releaseHostage()
			elseif IsDisabledControlJustPressed(0,79) then --kill
				holdingHostage = false
				holdingHostageInProgress = false
				local closestPlayer = GetClosestPlayer(2)
				target = GetPlayerServerId(closestPlayer)
...
function DrawText3Dotage(x,y,z, text)
