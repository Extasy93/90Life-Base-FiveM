...
local dbtxterr = "⚠️ • Vous devez avoir un sac pour sortir cette arme!"
local bwtxterr = "⚠️ • Cette arme ne peut être retirée que d'une voiture."
...
Citizen.CreateThread(function()
	while true do
		...
		if playerPed then
			...
			if daw == true then
				RemoveAllPedWeapons_(playerPed, true)
			else
				if isWeaponBig(weapon) then
					local vehicle = VehicleInFront()
					local bagIndex = GetPedDrawableVariation(playerPed, 5)
					local accessIndex = GetPedDrawableVariation(playerPed, 7)
					--if bagIndex == 40 or bagIndex == 41 or bagIndex == 44 or bagIndex == 45 or accessIndex == 11 or accessIndex == 12 or (accessIndex == 4 and GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_f_freemode_01")) then 
					if GetVehiclePedIsIn(playerPed, false) == 0 and DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) and bigWeaponOut == false then
						...
						if tafbw == true then
							local text = bwtxt
							--TriggerServerEvent('3dme:shareDisplay', token, text)
						end
						Citizen.Wait(2000)
						...
						if duffle == true then
							local bagIndex = GetPedDrawableVariation(playerPed, 5)
							local accessIndex = GetPedDrawableVariation(playerPed, 7)
							if bagIndex == 26 or bagIndex == 27 or bagIndex == 28 or bagIndex == 22 or bagIndex == 12 or bagIndex == 10 or bagIndex == 11 or accessIndex == 2 or accessIndex == 12 or accessIndex == 81 or accessIndex == 82 then 
							--if IsPedModel(playerPed,1885233650) and bigWeaponOut == false and GetVehiclePedIsIn(playerPed, false) == 0 then -- male
								--drawNotificationSac("* " ..GetPedDrawableVariation(playerPed,5).. "," ..GetPedTextureVariation(playerPed,5).. "," ..GetPedPaletteVariation(playerPed,5).. " *")
								...
								if IsPedModel(playerPed,-1667301416) and bigWeaponOut == false and GetVehiclePedIsIn(playerPed, false) == 0 then -- female
									...
									Wait(1)
									drawNotificationSac("", "", "~r~"..dbtxterr.."")
									SetCurrentPedWeapon(playerPed, -1569615261)
									end
								end
							end
							...
							if bigWeaponOut == false and GetVehiclePedIsIn(playerPed, false) == 0 then
								Wait(1)
								drawNotificationSac("", "", "~r~"..bwtxterr.."")
								SetCurrentPedWeapon(playerPed, -1569615261)
							end
						end
					end
...
function drawNotificationSac(title, subject, msg)
	local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(-1))
	Extasy.ShowAdvancedNotification(title, subject, msg, mugshotStr, 1) 
...
function getCustomization()
	...
	for i=0,20 do -- index limit to 20
		...
		drawNotificationSac("ped " ..GetPedDrawableVariation(ped,i).. " " ..GetPedTextureVariation(ped,i).. " " ..GetPedPaletteVariation(ped,i).. " ped")
	end

	-- props
	for i=0,10 do -- index limit to 10
		custom["p"..i] = {GetPedPropIndex(ped,i), math.max(GetPedPropTextureIndex(ped,i),0)}
		drawNotificationSac("props " ..GetPedPropIndex(ped,i).. " " ..math.max(GetPedPropTextureIndex(ped,i),0).. " props")
	end
