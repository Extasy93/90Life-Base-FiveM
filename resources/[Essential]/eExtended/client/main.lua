...
AddEventHandler('esx:addInventoryItem', function(item, count, showNotification, newItem)
	...
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item then
			ESX.ShowInventoryItemNotification(true, v.label, count - v.count)
	...
	if(found == false and newItem --[[Just a check if there is a newItem]])then
		...
		ESX.ShowInventoryItemNotification(true, newItem.label, count)
	else
	...
	if showNotification then
		ESX.ShowInventoryItemNotification(true, item, count)
	end
end)
...
AddEventHandler('esx:removeInventoryItem', function(item, count, showNotification)
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item then
			ESX.ShowInventoryItemNotification(false, v.label, v.count - count)
			ESX.PlayerData.inventory[k].count = count
			break
		end
	end
...
RegisterNetEvent('esx:spawnVehicles')
AddEventHandler('esx:spawnVehicles', function(vehicle)
...
AddPickup = function(pickupId, pickupLabel, pickupCoords, pickupType, pickupName, pickupComponents, pickupTint)
	pickups[pickupId] = {
		label = pickupLabel,
		textRange = false,
		coords = pickupCoords,
...
RegisterNetEvent('esx:deleteVehicles')
AddEventHandler('esx:deleteVehicles', function(radius)
	local playerPed = PlayerPedId()
