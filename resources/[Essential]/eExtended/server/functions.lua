...
ESX.RegisterCommand = function(name, group, cb, allowConsole, suggestion)
	...
	if ESX.RegisteredCommands[name] then
		print(('[ExtendedMode] [^3WARNING^7] Une commande "%s" est deja enregistree, commande prioritaire'):format(name))
...
ESX.TriggerServerCallback = function(name, requestId, source, cb, ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('[ExtendedMode] [^3WARNING^7] Le Server Callback "%s" n\'existe pas. Assurez-vous que le fichier cote serveur se charge reellement, une erreur dans ce fichier pourrait l\'empêcher de se charger.'):format(name))
	end
end
...
--[[ESX.SavePlayer = function(xPlayer, cb)
	...
	elseif ExM.DatabaseType == "newesx" then
		MySQL.Async.execute('UPDATE users SET accounts = @accounts, job = @job, job_grade = @job_grade, job2 = @job2, job2_grade = @job2_grade, `group` = @group, loadout = @loadout, position = @position, inventory = @inventory WHERE identifier = @identifier', {
...
ESX.SavePlayer = function(xPlayer, cb)
	...
	Async.parallel(asyncTasks, function(results)
		--print(('[extendedmode] [^2INFO^7] Sauvegarde de "^1%s^7"'):format(xPlayer.getName()))
...
ESX.SavePlayers = function(cb)
	...
	Async.parallelLimit(asyncTasks, 8, function(results)
		print(('[extendedmode] [^2INFO^7] Sauvegarde de ^1%s^7 joueur(s)'):format(#xPlayers))
		if cb then
			cb()
		end
	end)
...
--[[ESX.StartDBSync = function()
	function saveData()
		ESX.SavePlayers(function(result)
			if result then
				print('[ExtendedMode] [^2INFO^7] Sauvegarde automatique de toutes les donnees des joueurs')
			else
				print('[ExtendedMode] [^3WARNING^7] Echec de la sauvegarde automatique des donnees des joueurs ! Cela peut être du a une erreur interne sur le serveur MySQL.')
			end
		end)
		SetTimeout(10 * 60 * 1000, saveData)
	end
